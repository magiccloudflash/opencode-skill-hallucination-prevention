...
[English](./README.md) | [中文](./README_CN.md) | [Русский](./README_RU.md) | [Français](./README_FR.md) | [Deutsch](./README_DE.md) | [Español](./README_ES.md) | [Español (Cuba)](./README_CU.md) | [Português](./README_PT.md) | Português (Brasil) | [العربية](./README_AR.md)

## Funcionalidades

- **Verifique antes de afirmar** — leia arquivos, execute código, consulte documentação antes de concluir
- **Reconheça a incerteza** — distinga claramente entre especulação e fatos verificados
- **Não invente** — nunca fabrique erros, APIs, resultados de testes ou correções
- **Baseie-se em evidências** — cite caminhos de arquivos, números de linha e resultados reais
...
- **8 tipos de alucinações** — cada tipo tem sua própria estratégia de prevenção
...
Copie a habilidade para sua configuração global do OpenCode:
...
> O arquivo global `AGENTS.md` é carregado automaticamente em cada sessão, sem precisar invocar a habilidade manualmente.

## Uso

### Sob demanda (Método 1, 2)

O OpenCode descobre automaticamente a habilidade. O agente carrega quando necessário:
...
├── README_CU.md                              # Documentação em espanhol cubano
├── README_PT.md                              # Documentação em português (Portugal)
├── README_BR.md                              # Documentação em português brasileiro (este arquivo)
├── README_AR.md                              # Documentação em árabe
├── LICENSE                                   # Licença MIT
├── opencode.json                             # Exemplo de configuração
├── AGENTS.md                                 # Regras globais (carregamento automático)
└── hallucination-prevention/
    └── SKILL.md                              # Arquivo de definição da habilidade
```
...
|------|-----------|---------|-----------|
| **A: Factual** | Afirmar fatos não verificados | "Essa função retorna uma Promise" (sem ler o código) | Leia fontes, execute código |
| **B: Estrutural** | Inventar arquivos/caminhos | "O handler está em src/routes/auth.ts" (arquivo não existe) | Use glob/ls para verificar |
| **C: Comportamental** | Supor comportamento do código | "O bug foi corrigido" (sem executar o projeto) | Execute o código e mostre o resultado |
...
| **F: Temporal** | Versões/datas incorretas | "Node v24 adicionou essa funcionalidade" (v24 não existe) | Verifique versão e changelog |
...
| **H: Referência** | Inventar URLs/IDs/commits | "Veja https://github.com/foo/issues/99999" (link falso) | Verifique a URL ou indique "não verificado" |

### Regras por ferramenta

| Ferramenta | Regra principal |
|------------|-----------------|
| **read** | Não adivinhe o conteúdo; verifique existência do arquivo; releia após editar |
...
| **question** | Diante de instruções ambíguas, pergunte primeiro, aja depois |

### Lista de verificação de 10 pontos (o "checklist brabo")

Antes de cada resposta, verifique:

| # | Pergunta | Ação se "Não" |
|---|----------|---------------|
| 1 | Eu realmente LI os arquivos sobre os quais estou falando? | Leia-os agora |
| 2 | Eu realmente EXECUTEI os comandos cuja saída estou reportando? | Execute-os agora |
| 3 | Eu realmente VERIFIQUEI o pacote/API que estou referenciando? | Verifique ou marque como incerto |
| 4 | Tenho certeza que esta mensagem de erro é real? | Mostre a saída real |
| 5 | Releio os arquivos depois de editá-los? | Releia-os agora |
| 6 | Estou adivinhando a estrutura de diretórios? | Liste o diretório |
| 7 | Estou adivinhando uma assinatura de função? | Leia o código fonte |
| 8 | Afirmei que uma correção funciona sem testá-la? | Execute o código |
| 9 | Estou inventando opções de comandos? | Verifique --help |
| 10 | Minha resposta pode conter dados inventados? | Mantenha apenas a saída real |

### Protocolo de ancoragem contextual

1. **Antes de operações importantes com arquivos**: releia os arquivos fonte relevantes
2. **Após 5+ chamadas de ferramentas**: releia os arquivos modificados, verifique o estado acumulado
3. **Ao voltar a uma tarefa**: releia TODOS os arquivos relevantes — não confie na memória
...
Por padrão, use o nível **Verificado**.

### Protocolo de escalada (quando parar e perguntar ao usuário)

Pare imediatamente e pergunte ao usuário quando:
...
4. O observado contradiz a descrição do usuário
5. A tarefa requer conhecimento que você não pode verificar
6. A operação pode causar perda de dados ou problemas de segurança
7. O contexto mudou demais para ser seguido com precisão
...
Issues e Pull Requests são bem-vindas, meu consagrado!

https://github.com/magiccloudflash/opencode-skill-hallucination-prevention

---

**Navegação** : [English](./README.md) | [中文](./README_CN.md) | [Русский](./README_RU.md) | [Français](./README_FR.md) | [Deutsch](./README_DE.md) | [Español](./README_ES.md) | [Español (Cuba)](./README_CU.md) | [Português](./README_PT.md) | **Português (Brasil)** | [العربية](./README_AR.md)
